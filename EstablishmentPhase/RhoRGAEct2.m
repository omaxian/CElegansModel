...
Sigma0 = 10*2.6e-2;
% Reactions
K_EPM = 1;           % Active rho and myosin recruiting ECT-2 (relative to basal)
K_EP =  50;           % ECT-2  activating Rho
K_MP = 5;           % Actomyosin (RhoGAP) inhibiting Rho (relative to basal)
K_PM = 10;           % Rho producing actomyosin  
Kfb = 0.1;
...
for iT=0:nT
    ...
    % 3) Reaction
    ActivatingRho = K_EP*E.*PActive./(Kfb+PActive).*PInActive + ...
        -K_MP*M.*PActive;
    RHS_E = MinusdxEv + KonE*(~InPost + K_EPM*PActive.*M)*Ec - KoffE*E;
    RHS_PInAct = MinusdxPInActv + KonRho*Pc - KoffRho*PInActive - ActivatingRho;
    RHS_PAct = MinusdxPActv + ActivatingRho;
    RHS_M = MinusdxMv + KonM*(1 + K_PM*PActive)*Mc - M;
